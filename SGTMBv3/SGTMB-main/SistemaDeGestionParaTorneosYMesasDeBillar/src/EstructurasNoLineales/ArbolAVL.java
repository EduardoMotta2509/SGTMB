...
public class ArbolAVL<E extends Comparable<E>> implements BinarySearchTree<E> {
	...
	public String obtenerEstructuraVisual() {
        if (isEmpty()) return "El árbol AVL está vacío.";
        StringBuilder sb = new StringBuilder();
        sb.append("Estructura del Árbol (Rotado 90° hacia la izquierda):\n\n");
        generarEstructuraRec(this.root, 0, sb);
        return sb.toString();
    }

    private void generarEstructuraRec(NodeAVL nodo, int nivel, StringBuilder sb) {
        if (nodo == null) return;
        
        // Primero recorremos la rama derecha (que aparecerá arriba)
        generarEstructuraRec(nodo.right, nivel + 1, sb);
        
        // Tabulaciones según la profundidad del nodo
        for (int i = 0; i < nivel; i++) {
            sb.append("        "); 
        }
        
        // Imprimimos el nodo con su factor de balance actual
        int balance = getBalance(nodo);
        sb.append("┌── ").append(nodo.data.toString())
          .append(" [Bal: ").append(balance).append("]\n");
          
        // Luego recorremos la rama izquierda (que aparecerá abajo)
        generarEstructuraRec(nodo.left, nivel + 1, sb);
    }
}